...
<div class="max-w-4xl mx-auto">
    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Support Agent</h1>
        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Update agent information and permissions</p>
    </div>
    ...
    <div class="mb-6 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800 dark:border-blue-900/50 dark:bg-blue-950/50 dark:text-blue-200">
        <div class="flex gap-2">
            ...
            <div>
                <p class="font-medium">Employee ID is managed automatically</p>
                <p class="mt-1 text-xs opacity-90">Changing department will regenerate the Employee ID. The agent will be notified of the change.</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <form action="{{ route('admin.users.update', $user) }}" method="POST" id="editAgentForm">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="position" 
                            ...
                            <option value="Junior Support Agent" {{ old('position', $user->position) == 'Junior Support Agent' ? 'selected' : '' }}>Junior Support Agent</option>
                            <option value="Support Agent" {{ old('position', $user->position) == 'Support Agent' ? 'selected' : '' }}>Support Agent</option>
                            <option value="Senior Support Agent" {{ old('position', $user->position) == 'Senior Support Agent' ? 'selected' : '' }}>Senior Support Agent</option>
                            <option value="Team Lead" {{ old('position', $user->position) == 'Team Lead' ? 'selected' : '' }}>Team Lead</option>
                            <option value="Support Manager" {{ old('position', $user->position) == 'Support Manager' ? 'selected' : '' }}>Support Manager</option>
                        </select>
                    ...
                    <select name="category_ids[]" id="category_ids" multiple
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" 
                                    data-department-id="{{ $category->department_id }}"
            ...
            <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30 flex justify-end gap-3">
                ...
                <button type="submit" 
                    ...
                    <i class="bi bi-save mr-2"></i> Update Agent
                </button>
            </div>
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const originalEmployeeId = employeeIdField.value;
    
    // Store all category options
    const allCategories = Array.from(categorySelect.options);
    
    // Store currently selected category IDs
    let selectedCategoryIds = [];
    
    function updateSelectedCategories() {
        selectedCategoryIds = Array.from(categorySelect.options)
    ...
    function generateEmployeeId() {
        ...
        if (!departmentId) {
            employeeIdField.value = originalEmployeeId;
            return;
        }
    ...
    function filterCategoriesByDepartment() {
        ...
        allCategories.forEach(option => {
            ...
            if (!selectedDepartmentId || categoryDepartmentId === selectedDepartmentId) {
                ...
                if (selectedCategoryIds.includes(option.value) || option.selected) {
        ...
        // Update selected categories tracking
