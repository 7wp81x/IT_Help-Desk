...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Add a new department and its specializations to the system</p>
    </div>
                    ...
                    <div id="specializationsContainer" class="space-y-3">
                        ...
                        @for($i = 0; $i < $specCount; $i++)
                            <div class="specialization-item flex flex-col gap-2 sm:flex-row sm:items-start">
                                ...
                                <button type="button"
                                        class="remove-spec-btn mt-1 sm:mt-0 w-full sm:w-12 h-11 rounded-lg bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-200 transition flex items-center justify-center {{ $i === 0 ? 'opacity-50 cursor-not-allowed' : '' }}"
                                        {{ $i === 0 ? 'disabled' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let specCount = {{ count(old('specializations', [1])) }};
