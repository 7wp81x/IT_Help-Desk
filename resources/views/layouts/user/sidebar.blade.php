     ...
     <nav class="h-full flex flex-col bg-white dark:bg-gray-900">
        ...
        <div class="flex-1 px-3 py-4 overflow-y-auto">
            <ul class="space-y-1">
                ...
                <!-- My Tickets (Direct Link - NO DROPDOWN) -->
                <li>
                    <a href="{{ route('user.tickets.index') }}"
                        ...
                        :class="{
                            'justify-center': sidebarCollapsed,
                            'justify-start': !sidebarCollapsed,
                            'bg-blue-600 text-white shadow-md': activeMenu === 'my-tickets',
                        ...
                        @mouseenter="sidebarCollapsed ? showTooltip($event, 'My Tickets') : null"
                        @mouseleave="hideTooltip()">
                        <i class="bi bi-ticket-detailed text-xl"></i>
                        <span class="text-sm font-medium" :class="{ 'hidden': sidebarCollapsed }">My Tickets</span>
                        <span class="ml-auto bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs rounded-full px-2 py-0.5">{{ $ticketCounts['my_tickets'] ?? 0 }}</span>
                    </a>
                </li>

                <!-- New Ticket (Direct Link - NO DROPDOWN) -->
...
<li>
    <a href="{{ route('user.tickets.create') }}"
        ...
        @mouseenter="sidebarCollapsed ? showTooltip($event, 'New Ticket') : null"
        @mouseleave="hideTooltip()">
        <i class="bi bi-plus-circle text-xl" :class="{ 'text-white': activeMenu === 'create-ticket' }"></i>
        <span class="text-sm font-medium" :class="{ 'hidden': sidebarCollapsed }">New Ticket</span>
    </a>
</li>

                <!-- Knowledge Base -->
                <li>
                    <a href="{{ route('user.knowledgebase') }}"
                        class="flex items-center gap-3 p-2.5 rounded-lg transition-all duration-200"
                        :class="{
                            'justify-center': sidebarCollapsed,
                            'justify-start': !sidebarCollapsed,
                            'bg-purple-600 text-white shadow-md': activeMenu === 'knowledgebase',
                            'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800': activeMenu !== 'knowledgebase'
                        }"
                        @click="setActiveMenu('knowledgebase')"
                        @mouseenter="sidebarCollapsed ? showTooltip($event, 'Knowledge Base') : null"
                        @mouseleave="hideTooltip()">
                        <i class="bi bi-journal-bookmark-fill text-xl" :class="{ 'text-white': activeMenu === 'knowledgebase' }"></i>
                ...
                <li>
                    <a href="{{ route('user.announcements') }}"
                        ...
                        :class="{
                            'justify-center': sidebarCollapsed,
                            'justify-start': !sidebarCollapsed,
                            'bg-cyan-600 text-white shadow-md': activeMenu === 'announcements',
                            'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800': activeMenu !== 'announcements'
                        }"
                        @click="setActiveMenu('announcements')"
                        @mouseenter="sidebarCollapsed ? showTooltip($event, 'Announcements') : null"
                        @mouseleave="hideTooltip()">
                        <i class="bi bi-megaphone text-xl" :class="{ 'text-white': activeMenu === 'announcements' }"></i>
                        <span class="text-sm font-medium" :class="{ 'hidden': sidebarCollapsed }">Announcements</span>
                    </a>
                </li>
                ...
                <li x-data="{ supportOpen: false }" x-init="supportOpen = activeMenu === 'support'">
                    <button @click="if (!sidebarCollapsed) { supportOpen = !supportOpen; if(supportOpen) setActiveMenu('support') }"
                            ...
                            @mouseenter="sidebarCollapsed ? showTooltip($event, 'Support') : null"
                            @mouseleave="hideTooltip()">
                    ...
                    <ul x-show="supportOpen && !sidebarCollapsed" 
                        x-cloak
                        ...
                        <li>
                            <a href="{{ route('user.support') }}"
                                ...
                                @click="setActiveMenu('support'); setActiveSubMenu('faq'); supportOpen = true">
                        ...
                        <li>
                            <a href="{{ route('user.agents') }}"
                                ...
                                @click="setActiveMenu('support'); setActiveSubMenu('agents'); supportOpen = true">
                        ...
                        <li>
                            <a href="{{ route('user.ratings') }}"
                                ...
                                @click="setActiveMenu('support'); setActiveSubMenu('ratings'); supportOpen = true">
...
function userSidebarComponent() {
    return {
        ...
        updateActiveMenuFromRoute() {
            ...
            } else if (currentPath.includes('/user/knowledgebase')) {
                this.activeMenu = 'knowledgebase';
            } else if (currentPath.includes('/user/announcements')) {
