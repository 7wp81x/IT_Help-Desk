...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">My Assigned Tickets</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 mb-6" id="stats-container">
        <a href="#" data-status="all" 
           class="stat-card bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all hover:scale-105 cursor-pointer group">
        ...
        <a href="#" data-status="open" 
           class="stat-card bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all hover:scale-105 cursor-pointer group">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Open</p>
                    <p class="text-2xl font-bold text-yellow-600 mt-1 stat-open">{{ $stats['open'] ?? $openCount }}</p>
                </div>
           ...
           class="stat-card bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all hover:scale-105 cursor-pointer group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-orange-600 mt-1 stat-pending">{{ $stats['pending'] ?? $pendingCount }}</p>
                </div>
           ...
           class="stat-card bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all hover:scale-105 cursor-pointer group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-blue-600 mt-1 stat-progress">{{ $stats['in_progress'] ?? $inProgressCount }}</p>
                </div>
                <div class="w-12 h-12 bg-blue-100 dark:bg-blue-900/30 rounded-xl flex items-center justify-center group-hover:bg-blue-200 dark:group-hover:bg-blue-800/50 transition-colors">
        ...
        <a href="#" data-status="resolved" 
           class="stat-card bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all hover:scale-105 cursor-pointer group">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Resolved</p>
                    <p class="text-2xl font-bold text-green-600 mt-1 stat-resolved">{{ $stats['resolved'] ?? $resolvedCount }}</p>
                </div>
           ...
           class="stat-card bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all hover:scale-105 cursor-pointer group">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-600 dark:text-gray-400 mt-1 stat-closed">{{ $stats['closed'] ?? $closedCount }}</p>
                </div>
        ...
        <a href="#" data-status="canceled" 
           class="stat-card bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all hover:scale-105 cursor-pointer group">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Canceled</p>
                    <p class="text-2xl font-bold text-gray-600 mt-1 stat-canceled">{{ $stats['canceled'] ?? $canceledCount }}</p>
                </div>
                <div class="w-12 h-12 bg-gray-100 dark:bg-gray-700 rounded-xl flex items-center justify-center group-hover:bg-gray-200 dark:group-hover:bg-gray-600 transition-colors">
                ...
                <div class="w-[140px]">
                    <div class="relative">
                        ...
                        <select id="priorityFilter" class="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 cursor-pointer">
                            <option value="" {{ request('priority') === null ? 'selected' : '' }}>All Priorities</option>
                ...
                <div class="w-[160px]">
                    <div class="relative">
                        ...
                        <select id="categoryFilter" class="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 cursor-pointer">
                            <option value="" {{ request('category') === null ? 'selected' : '' }}>All Categories</option>
    ...
    <div id="tableContainer">
        @include('agent.tickets.partials.table', ['tickets' => $tickets])
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    statCards.forEach(card => {
        card.addEventListener('click', function(e) {
            ...
            // Clear other filters for better UX
    ...
    function fetchTickets() {
        ...
        fetch(`{{ route('agent.tickets.index') }}?${params.toString()}`, {
            headers: {
