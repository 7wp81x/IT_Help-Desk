<div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
    ...
    <div class="p-5 max-h-96 overflow-y-auto space-y-4" data-comments-container>
        @forelse($ticket->comments as $comment)
            @include('user.tickets.partials.comment-item', ['comment' => $comment])
        @empty
    ...
    <!-- Comment Form -->
    @if(!in_array($ticket->status, ['closed', 'canceled']))
        @include('user.tickets.partials.comment-form', ['ticket' => $ticket])
    @endif
</div>