        ...
        @if($imageAttachments->count() > 0)
            <div class="flex flex-wrap gap-2">
                @foreach($imageAttachments as $attachment)
                    <div class="relative group cursor-pointer" onclick="openImagePreview(event, '{{ route('user.tickets.download', $attachment) }}', '{{ $attachment->original_name }}')">
                        <img src="{{ route('user.tickets.download', $attachment) }}" 
        ...
        @if($documentAttachments->count() > 0)
            <div class="space-y-1">
                @foreach($documentAttachments as $attachment)
                    ...
                    @endphp
                    <div class="flex items-center justify-between bg-white dark:bg-gray-800 rounded px-2 py-1.5 border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition cursor-pointer" onclick="openAttachmentPreview(event, '{{ route('user.tickets.download', $attachment) }}', '{{ $attachment->original_name }}')" title="Click to preview">
                        ...
                        <div class="flex-shrink-0 ml-2 flex gap-1">
                            <button type="button" onclick="event.stopPropagation(); openAttachmentPreview(event, '{{ route('user.tickets.download', $attachment) }}', '{{ $attachment->original_name }}')" class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 p-1" title="Preview">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                </svg>
                            </button>
                            <a href="{{ route('user.tickets.download', $attachment) }}" class="flex-shrink-0 text-green-600 hover:text-green-700 dark:text-green-400 dark:hover:text-green-300 p-1" title="Download">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
