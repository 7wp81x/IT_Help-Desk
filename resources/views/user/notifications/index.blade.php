    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                ...
                @if($readCount > 0)
                <form method="POST" action="{{ route('user.notifications.delete-all') }}" class="inline">
                    @csrf
                    <button type="submit" 
    ...
    <div class="space-y-2">
        @forelse($notifications as $notification)
            @php
                ...
                // Default action URL
                $actionUrl = route('user.notifications.show', $notification->id);
                
                // Check for ticket-related notifications
                if (str_contains($type, 'Ticket') || str_contains($type, 'ticket') || str_contains($type, 'TicketCreated')) {
                    $ticketId = data_get($data, 'ticket_id');
                    if ($ticketId) {
                        $actionUrl = route('user.tickets.show', $ticketId);
                    }
                    $icon = 'bi-ticket';
            ...
            <div class="group relative">
                ...
                <a href="{{ $actionUrl }}" 
                   class="block rounded-xl transition-all duration-200 {{ $read ? 'hover:bg-gray-50 dark:hover:bg-gray-800/50' : 'bg-blue-50/50 dark:bg-blue-900/20 hover:bg-blue-50 dark:hover:bg-blue-900/30' }}">
